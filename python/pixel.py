...
# 初始化画布
canvas = [[COLORS['白色'] for _ in range(CANVAS_SIZE)] for _ in range(CANVAS_SIZE)]
...
# 保存画布为图像
def save_canvas():
    # 创建一个新的surface来保存画布内容
    save_surface = pygame.Surface((CANVAS_SIZE, CANVAS_SIZE))
    
    # 将颜色数据复制到surface
    for y in range(CANVAS_SIZE):
        for x in range(CANVAS_SIZE):
            save_surface.set_at((x, y), canvas[y][x])
